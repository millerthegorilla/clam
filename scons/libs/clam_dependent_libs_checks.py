...
#			'-march=nocona',
	...
	if env['release'] :
		...
		env.AppendUnique( CCFLAGS='-g -O3 -fomit-frame-pointer -Wall -fpermissive -std=c++0x'.split(' ') )
	else :
		print 'COMPILING IN DEBUG MODE'
		env.AppendUnique( CCFLAGS='-g -Wall -fpermissive -std=c++0x'.split(' ') )
...
int main()
{
	client = jack_client_open ("foo", JackUseExactName, NULL);
	if ( client != NULL )
